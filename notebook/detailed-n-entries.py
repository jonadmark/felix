...
class ForwardingTactic:
    ...
    def __init__(self, id, n_entries, nexthops, prevhops, reachability, dist, n_transition_entries=None, failure_set=None, detailed_n_entries=None):
        self.id = id
        ...
        self.detailed_n_entries = detailed_n_entries
...
def cnft(nm, graph, dst):
    ...
    # return (n_entries, nexthops, prevhops, reachability, list(dist))
    return (n_entries, nexthops, prevhops, reachability, dist2)
...
def caft(nm, graph, nft, ftid, failure_set):
        ...
        if recompute:
            ...
            while heapq:
                ...
                if nexthops[dst][u] and not nexthops[dst][u].issuperset(nft.nexthops[dst][u]):
                    n_entries[u] = n_entries[u] + 1
                    if u not in detailed_n_entries:
                        detailed_n_entries[u] = 0
                    detailed_n_entries[u] = detailed_n_entries[u] + 1
                ## simpler version
    ...
    return ForwardingTactic(ftid, n_entries, None, None, reachability, dist, n_transition_entries, failure_set, detailed_n_entries)
...
def caft_worker(nm, graph, nft, worker_id, n_workers):
    ...
    for i, link in enumerate(graph.edges):
        if (i % n_workers) == worker_id:
            ...
            detailed_n_entries[ftid] = aft.detailed_n_entries
            failure_sets[ftid] = aft.failure_set
    for i, node in enumerate(graph.nodes):
        if ((m + i) % n_workers) == worker_id:
            ...
            detailed_n_entries[ftid] = aft.detailed_n_entries
            failure_sets[ftid] = aft.failure_set
    return (detailed_n_entries, failure_sets, worker_n_entries, worker_n_transition_entries)
...
def evaluate(nm, graph, n_workers):
    ...
    detailed_n_entries = {}
    failure_sets = {}
    for w_detailed_n_entries, w_failure_sets, w_n_entries, w_n_transition_entries in workers_results:
        detailed_n_entries.update(w_detailed_n_entries)
        failure_sets.update(w_failure_sets)
        global_n_entries = global_n_entries + w_n_entries
        global_n_transition_entries = global_n_transition_entries + w_n_transition_entries
    ...
    for ds in datasets:
        ...
        for nn in ds['networks']:
            ...
            with open(f"{base_dir}/{ds['directory']}/{nn}.graph.json") as f:
                nm = json.load(f)
            dne, _ = proc_network(nm, n_workers)
            # print(dne)
            # print(cdf)
            with open(f"./results/routing/detailed-n-entries/{ds['name']}-{nn}.json", "w") as f:
                # json.dump(dne, f, indent=4)
                json.dump(dne, f)
            # cdf.to_csv(f"./csvs/detailed_n_entries/{ds['name']}-{nn}.csv")
            print()
    return df
